  ...
  <div class="container mx-auto p-4 dark:text-white" id="containerOne">
    <div class="overflow-x-auto mt-1">
      <div class="flex items-center justify-center mb-8">
        ...
        <h2 class="text-3xl font-bold">Declarações do MEI</h2>
      </div>
      <p class="text-center text-gray-500 mb-8">Gerencie as declarações das empresas cadastradas no MEI de forma fácil e eficiente.</p>
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex space-x-4">
          <div class="buttonAlternar">
            <router-link to="/mei_impostos" class="bg-blue-500 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded bg-sky-400">
              Ir para Impostos
            </router-link>
          </div>
...
export default {
  ...
  methods: {
    fetchDeclaracoes() {
      ...
      axios.get(`${apiUrl}/backend/dp_fiscal/mei/listarDeclaracoesMei.php`)
        .then(response => {
    ...
    removeDeclaracao(declaracao) {
      ...
      axios.post(`${apiUrl}/backend/funcionalidades/remove_declaracao.php`, {
        ...
        .then(response => {
          ...
          this.declaracoes = this.declaracoes.filter(i => !(i.idEmpresa === declaracao.idEmpresa && i.idDeclaracao === declaracao.idDeclaracao && i.mes === declaracao.mes));
        })
        .catch(error => {
    ...
    saveFilters() {
      ...
      localStorage.setItem('filters_mei_declaracoes', JSON.stringify(filtersToSave));
    },
    loadFilters() {
      const savedFilters = localStorage.getItem('filters_mei_declaracoes');
      if (savedFilters) {
        this.filters = { ...this.filters, ...JSON.parse(savedFilters) };
      }
    },
    saveFavoritos() {
      localStorage.setItem('favoritos_mei_declaracoes', JSON.stringify(this.favoritos));
    },
    loadFavoritos() {
      const savedFavoritos = localStorage.getItem('favoritos_mei_declaracoes');
      if (savedFavoritos) {
