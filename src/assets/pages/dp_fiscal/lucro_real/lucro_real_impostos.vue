  ...
  <div class="container mx-auto p-4 dark:text-white" id="containerOne">
    <div class="overflow-x-auto mt-1">
      <div class="flex items-center justify-center mb-8">
        ...
        <h2 class="text-3xl font-bold">Impostos do Lucro Real</h2>
      </div>
      <p class="text-center text-gray-500 mb-8">Gerencie os impostos das empresas cadastradas no Lucro Real de forma fácil e eficiente.</p>
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex space-x-4">
          <div class="buttonAlternar">
            <router-link to="/lucro_real" class="bg-blue-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded bg-yellow-600">
              Ir para Declarações
            </router-link>
          </div>
          <!-- Mês -->
...
export default {
  ...
  methods: {
    fetchImpostos() {
      ...
      axios.get(`${apiUrl}/backend/dp_fiscal/lucro_real/lr_impostos.php`)
        .then(response => {
  ...
  .catch(error => {
    ...
    openModal(imposto, idEmpresa, mes) {
    ...
    saveFilters() {
      ...
      localStorage.setItem('filters_lucro_real_impostos', JSON.stringify(filtersToSave));
    },
    loadFilters() {
      const savedFilters = localStorage.getItem('filters_lucro_real_impostos');
      if (savedFilters) {
        this.filters = { ...this.filters, ...JSON.parse(savedFilters) };
      }
    },
    saveFavoritos() {
      localStorage.setItem('favoritos_lr_impostos', JSON.stringify(this.favoritos));
    },
    loadFavoritos() {
      const savedFavoritos = localStorage.getItem('favoritos_lr_impostos');
      if (savedFavoritos) {
