      ...
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex space-x-4">
          <div class="buttonAlternar">
            <router-link to="/lucro_presumido_impostos" class="bg-blue-500 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded bg-sky-400">
              Ir para Impostos
            </router-link>
          </div>
          <!-- Mês -->
...
export default {
  ...
  methods: {
    ...
    saveFilters() {
      ...
      localStorage.setItem('filters_lucro_presumido', JSON.stringify(filtersToSave));
    },
    loadFilters() {
      const savedFilters = localStorage.getItem('filters_lucro_presumido');
      if (savedFilters) {
